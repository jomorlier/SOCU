...
#' @param modelType    Type of the model which cann be "rbf" or "kriging"                   
#' @param model        the constraint surrogate models 
...
#'       \item{\code{fn}}{   true functions, only needed in case of \code{checkit==TRUE} or 
...
interpRBF<-function(x,con.model,SORC){
  #browser()
  x<-t(as.matrix(x))
 # colnames(x) = colnames(con.model[[1]]@X)
  colnames(x) =paste("x",c(1:length(x)),sep="")
  PRED<-c()
  for(con in c(1:SORC$nConstraints)){
    model<-con.model[[con]]
    colnames(x) = colnames(model@X)
    predx <- predict(object = model, newdata = x, type = SORC$kType, 
                     checkNames = FALSE)
    PRED<-c(PRED,predx$mean)
...
repairInfeasRI2 <- function(x,gReal,rbf.model,cobra,checkIt=FALSE,modelType="kriging"){  
 ...
 # browser()
  ...
  if (is.null(gradEps)) gradEps = min(upperP-lowerP)/100000; 
 ...
 # equ2Index <- c(cobra$equIndex,cobra$nConstraints+(1: length(cobra$equIndex)))
  ...
  isEpsilonFeasible <- function(y,eps2,rbf.model) {
    if (!trueFunc) {
      fRbf <- interpRBF(y,rbf.model,cobra)
    } else {
      fRbf <- conFunc(y)      
    }
...
#     }
  ...
  findBestInfeasible <- function(deltaMat,x,eps2,rbf.model) {
    numMaxViol <- function(k) {
      if (!trueFunc) {
        fRbf <- interpRBF(x+deltaMat[k,],rbf.model,cobra)
      } else {
...
#       }
  ...
  checkSingleConstraints <- function(Del,Grad,Viol) {
    ...
    for (k in 1:length(Viol)) {
      v = Viol[k]
      fBefore = c(fBefore,interpRBF(x,rbf.model,cobra)[v])
      fSingle = c(fSingle,interpRBF(x+Del[k,],rbf.model,cobra)[v])
    }
  ...
  checkBestInfeasible <- function(Del) {
    ...
    fx=interpRBF(x,rbf.model,cobra); indx=which(fx+ri$eps2>0);        # infeasible point
    fy=interpRBF(x+Delta,rbf.model,cobra); indy=which(fy+ri$eps2>0);  # best infeasible repair 
    fw=interpRBF(x_old,rbf.model,cobra); indw=which(fw+ri$eps2>0);    # old repair mechanism
...
#  colnames(nd) = colnames(rbf.model[[1]]@X)
  
  if (!trueFunc) {
    f<-apply(nd,1,FUN=function(x){
      interpRBF(x,rbf.model,cobra)
    })
    f<-matrix(f,nrow=nrow(nd))
    #if(cobra$nConstraints==1)f<-matrix(1)
  ...
  myFn<-function(x,k,con.model,SORC){
    res<-interpRBF(x,con.model=con.model,SORC=SORC)[k]
    return(res)
    }
  ...
  while(1) {
    ...
    Viol <- NULL
    for (k in c(1:ncol(f))) {
      if (gReal[k]+ri$eps1>0) {
        Del_k<-grad(myFn,x,k=k,con.model=rbf.model,SORC=cobra)
        Del<-rbind(Del,-(Del_k*1e-3)/sqrt(sum(Del_k^2)))
      }  
    } #for k (k:number of constraints)
...
#         g2 <- sum(gradf*gradf)  
#         if (g2==0) {
#           print("Cannot repair infeasible solution w/o moving out of search region")
...
#     } # for k
    ...
    if (ri$OLD) 
    {
      ...
      Delta = colSums(Del)
      #ri$q<-max((10^-2)/sqrt(sum(Del^2)),ri$q)
     # browser()
      ...
      #SB: only for testing
      #HYPERSPHERE<-F
      #if(sqrt(sum(Del^2))<1e-6){
     #   HYPERSPHERE<-T
     #   print("very small parallelepiped")
     # }
      for (m in c(1:ri$mmax)) {
          alpha = runif(nrow(Del),min = ri$eps3, max = 1)*ri$q         # random coef. from distribution U[0,a]
      ...
      if(is.null(S)){
        for (m in c(1:ri$mmax)) {
          alpha = runif(nrow(Del),min = ri$eps3, max = 1)*ri$q *2        # random coef. from distribution U[0,a]
          alphaMat = outer(alpha,rep(1,ncol(Del)))
          delta = colSums(alphaMat*Del)
          deltaMat = rbind(deltaMat,delta)
      ...
      if(is.null(S)){
      #  browser()
        print("repair could not find a feasible solution in paralellepiped on the surrogate")
        
        for (m in c(1:ri$mmax)) {
        DeltaLength<-sqrt(sum(Delta^2))
        delta<-rnorm(ncol(Del),mean=0,sd=1e-8)
        deltaMat = rbind(deltaMat,delta)
        if (isEpsilonFeasible(x+delta,ri$eps2,rbf.model))
          S = rbind(S,delta)
        }
        ...
        if (is.null(S)) {
          ...
          print("repair could not find a feasible solution on the surrogate")
 ...
 # browser()
  ...
  if (checkIt) {
    fRbf = interpRBF(z,rbf.model,cobra)       # fRbf:  constraint surrogate values after repair
